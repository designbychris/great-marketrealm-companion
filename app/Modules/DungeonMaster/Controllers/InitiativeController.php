...
final class InitiativeController
{
    ...
    public function update(string $id, string $encounterId, SaveInitiativeRequest $request): RedirectResponse
    {
        ...
        if($action==='complete'){ $encounter->update($encounter->title(),$encounter->sessionId(),Encounter::STATUS_COMPLETED,$encounter->threat(),$encounter->location(),$encounter->adversaries(),$encounter->notes(),$encounter->characterIds(),$encounter->monsterGroups()); $this->encounters->save($encounter,$campaign); }
        elseif($encounter->status()===Encounter::STATUS_PREPARED){ $encounter->update($encounter->title(),$encounter->sessionId(),Encounter::STATUS_RUNNING,$encounter->threat(),$encounter->location(),$encounter->adversaries(),$encounter->notes(),$encounter->characterIds(),$encounter->monsterGroups()); $this->encounters->save($encounter,$campaign); }
    ...
    private function seedCombatants(Encounter $encounter, Campaign $campaign): array
    {
        ...
        foreach($encounter->monsterGroups() as $group){if(!is_array($group)){continue;}$monsterId=sanitize_text_field((string)($group['monster_id']??''));$name=sanitize_text_field((string)($group['name']??'Creature'));$quantity=max(1,min(20,(int)($group['quantity']??1)));$maxHp=max(1,min(99999,(int)($group['max_hp']??1)));$modifier=max(-20,min(20,(int)($group['initiative_modifier']??0)));for($i=1;$i<=$quantity;$i++){$label=$quantity>1?$name.' #'.$i:$name;$out[]=['id'=>'monster-'.substr(sha1($monsterId.'-'.$i),0,12),'type'=>'adversary','source_id'=>$monsterId,'name'=>$label,'initiative'=>0,'modifier'=>$modifier,'armor_class'=>max(0,min(99,(int)($group['armor_class']??0))),'challenge'=>sanitize_text_field((string)($group['challenge']??'')),'current_hp'=>$maxHp,'max_hp'=>$maxHp,'conditions'=>'','defeated'=>false];}}
