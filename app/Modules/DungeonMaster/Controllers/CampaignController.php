...
{
 public function __construct(private CampaignRepository $campaigns,private DungeonMasterAccess $access,private ViewFactory $views,private ResponseFactory $responses,private FlashStore $flash,private CampaignCommandCentre $commandCentre,private ?CampaignExpansionAccess $expansionAccess=null){}
 ...
 public function show(string $id): string { $this->guard(); $campaign=$this->campaign($id); return $this->views->render(View::make('dungeonmaster.campaigns.show',['campaign'=>$campaign,'commandCentre'=>$this->commandCentre->build($campaign),'expansionShelf'=>$this->expansionAccess?->shelf($campaign)??[],'flash'=>['success'=>$this->flash->get('success'),'error'=>$this->flash->get('error')]])); }
