...
final class DungeonMasterServiceProvider extends ServiceProvider
{
 public function register(): void {
  ...
  $this->app->bind(CampaignController::class,static fn(Container $c): CampaignController=>new CampaignController($c->make(CampaignRepository::class),$c->make(DungeonMasterAccess::class),$c->make(ViewFactory::class),$c->make(ResponseFactory::class),$c->make(FlashStore::class),$c->make(CampaignCommandCentre::class),$c->make(CampaignExpansionAccess::class)));
  $this->app->bind(CampaignExpansionController::class,static fn(Container $c): CampaignExpansionController=>new CampaignExpansionController($c->make(CampaignRepository::class),$c->make(CampaignExpansionAccess::class),$c->make(DungeonMasterAccess::class),$c->make(ResponseFactory::class),$c->make(FlashStore::class)));
 ...
 public function boot(): void
 {
  ...
  add_filter('gmrc_tabletop_bestiary_records',[$this,'tabletopBestiaryRecords']);
  $bridge=$this->app->make(TabletopSessionBridge::class);
  add_filter('gmrt_companion_campaign_choices',[$bridge,'campaignChoices'],10,2);
  add_filter('gmrt_companion_campaign_for_table',[$bridge,'campaignForTable'],10,3);
  add_filter('gmrt_companion_link_campaign',[$bridge,'linkCampaign'],10,4);
  add_filter('gmrt_companion_sync_table_session',[$bridge,'synchroniseSession'],10,3);

  /*
   * Scope expansion Character content to the current Guild member. DMs see
   * their available active Almanacs; Players inherit the union shared by the
   * active Campaigns whose rosters contain them.
   */
  add_filter(
   'gmrc_expansion_character_scope',
   function (mixed $scope): array {
    $userId = function_exists('get_current_user_id')
     ? (int) get_current_user_id()
     : 0;

    return $this->app->make(CampaignExpansionAccess::class)
     ->expansionKeysForUser($userId);
   }
  );
 }
