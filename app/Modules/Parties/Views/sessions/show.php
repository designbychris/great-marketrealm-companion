...
<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><?php wp_nonce_field('gmrc_party_session_note_'.$party->id()->value().'_'.$sessionId,'gmrc_nonce'); ?><input type="hidden" name="action" value="gmrc_app_request"><input type="hidden" name="gmrc_route" value="parties/<?php echo esc_attr($party->id()->value()); ?>/sessions/<?php echo esc_attr($sessionId); ?>/notes"><label>Add a memory<textarea name="content" rows="4" maxlength="3000" required></textarea></label><button type="submit">Add to this Session</button></form></section>
