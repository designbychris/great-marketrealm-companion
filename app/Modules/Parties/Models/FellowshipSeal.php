...
/**
 * Short-lived invitation seal for joining an existing Fellowship.
 */
final class FellowshipSeal
{
    ...
    private function __construct(
        ...
        if (strlen(self::normalise($code)) !== 8) {
            throw new InvalidArgumentException(
                'A Fellowship Seal requires an eight-character invite code.'
            );
        }

        if ($issuedAt < 1 || $expiresAt <= $issuedAt) {
            throw new InvalidArgumentException(
                'A Fellowship Seal requires a valid issue and expiry window.'
            );
        }
    }
    ...
    {
        return self::normalise($this->code);
    }
    ...
    {
        return strtoupper(
            (string) preg_replace('/[^A-Z0-9]/i', '', trim($code))
        );
    }
