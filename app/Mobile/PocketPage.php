...
function pocketDiceworks(){
 ...
 const count=field('Number of dice',1,1,20),modifier=field('Modifier',0,-999,999);body.append(fields);
 ...
 roll.addEventListener('click',()=>{
  const n=Number(count.value),mod=Number(modifier.value);
  if(!count.validity.valid||!modifier.validity.valid||!Number.isSafeInteger(n)||n<1||n>20||!Number.isSafeInteger(mod)||mod< -999||mod>999){result.textContent='Enter 1–20 dice and a whole-number modifier from −999 to +999.';return;}
  try{const values=[];for(let i=0;i<n;i++)values.push(pocketSecureDie(sides));const total=values.reduce((sum,value)=>sum+value,mod);
   result.replaceChildren(el('strong','',total),el('div','',n+'d'+sides+(mod>=0?' + ':' − ')+Math.abs(mod)+' · ['+values.join(', ')+']'));
   if(sides===20&&n===1&&values[0]===20)result.append(el('div','','✨ Natural 20! ✨'));
   if(sides===20&&n===1&&values[0]===1)result.append(el('div','','🎲 Natural 1!'));
   const entry=el('li','',n+'d'+sides+(mod>=0?' + ':' − ')+Math.abs(mod)+' → '+total+' ('+values.join(', ')+')');history.prepend(entry);while(history.children.length>6)history.lastElementChild.remove();
