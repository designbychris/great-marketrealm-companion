...
function pocketDiceworks(){
 const tray=el('details','gmrc-pocket-dice'),summary=el('summary','','Guild Diceworks 🎲');tray.append(summary);
 ...
 function performRoll(die,n,mod,label){
  ...
  try{const values=[];for(let i=0;i<n;i++)values.push(pocketSecureDie(die));const total=values.reduce((sum,value)=>sum+value,mod);
   const descriptor=label||n+'d'+die;result.replaceChildren(el('strong','',total),el('div','',descriptor+(mod>=0?' + ':' − ')+Math.abs(mod)+' · ['+values.join(', ')+']'));
...
function pocketDashboard(detail,character,back,diceTray){
 ...
 const tabs=el('div','gmrc-pocket-dashboard__tabs');tabs.setAttribute('role','tablist');tabs.setAttribute('aria-label','Character sections');
 const sections=[['overview','Overview',[vitality]],['character','Character',[abilityHeading,abilities,saving,skills]],['combat','Combat',[arsenal]],['dice','Diceworks',[diceTray]],['spells','Spellbook',[spellbook]],['equipment','Equipment',[]]];
 const panels=new Map(),buttons=new Map();
 for(const [key,label,nodes] of sections){
  const button=el('button','gmrc-pocket-dashboard__tab',label);button.type='button';button.setAttribute('role','tab');button.id=root.id+'-tab-'+key;button.setAttribute('aria-controls',root.id+'-panel-'+key);button.setAttribute('aria-selected',key==='overview'?'true':'false');button.tabIndex=key==='overview'?0:-1;
  const panel=el('section','gmrc-pocket-dashboard__panel');panel.id=root.id+'-panel-'+key;panel.setAttribute('role','tabpanel');panel.setAttribute('aria-labelledby',button.id);panel.hidden=key!=='overview';
