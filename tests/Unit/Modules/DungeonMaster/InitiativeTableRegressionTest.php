...
final class InitiativeTableRegressionTest extends TestCase
{
    ...
    public function testCombatantsSeedOnlyFromEncounterRosterAndAdversaries(): void { $s=$this->source('app/Modules/DungeonMaster/Controllers/InitiativeController.php');$c=preg_replace('/\s+/','',$s);self::assertIsString($c);self::assertStringContainsString('$encounter->characterIds()',$s);self::assertStringContainsString('$this->rosters->members($campaign)',$s);self::assertStringContainsString('findForOwner(CharacterId::fromString',$c);self::assertStringContainsString('$encounter->adversaries()',$s); }
    public function testCharacterHpAndInitiativeAreSeededWithoutMutatingCharacter(): void { $s=$this->source('app/Modules/DungeonMaster/Controllers/InitiativeController.php');self::assertStringContainsString('->hitPoints()',$s);self::assertStringContainsString('->initiative()->value()',$s);self::assertStringNotContainsString('$this->characters->update',$s);self::assertStringNotContainsString('$this->characters->save',$s); }
    public function testInitiativeSupportsSortRoundsTurnsConditionsAndCompletion(): void { $s=$this->source('app/Modules/DungeonMaster/Controllers/InitiativeController.php');$c=preg_replace('/\s+/','',$s);self::assertIsString($c);self::assertStringContainsString("\$action==='sort'",$c);self::assertStringContainsString("\$action==='advance'",$c);self::assertStringContainsString("\$action==='complete'",$c);self::assertStringContainsString("'conditions'",$s);self::assertStringContainsString('Encounter::STATUS_COMPLETED',$s); }
